...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        s_nodes = set()
        a_levels = self.a_levels[level-1]
        for a_node in a_levels:
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        eff_1 = [x for x in node_a1.action.effect_add if x in node_a2.action.effect_rem]
        eff_2 = [x for x in node_a2.action.effect_add if x in node_a1.action.effect_rem]
        
        if eff_1 or eff_2: return True
        else: return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        eff_1 = [x for x in node_a1.action.effect_add if x in node_a2.action.precond_neg]
        eff_2 = [x for x in node_a2.action.effect_add if x in node_a1.action.precond_neg]
        eff_3 = [x for x in node_a1.action.effect_rem if x in node_a2.action.precond_pos]
        eff_4 = [x for x in node_a2.action.effect_rem if x in node_a1.action.precond_pos]
        if eff_1 or eff_2 or eff_3 or eff_4: return True
        else: return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for p1, p2 in zip(node_a1.parents, node_a2.parents):
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for p1, p2 in zip(node_s1.parents, node_s2.parents):
        ...
        while goals:
            for level in range(len(self.s_levels)):
                ...
                for i, state in enumerate(self.s_levels[level]):
                    if state.is_pos: s.append(state.symbol)
                for goal in goals:
                    if goal in s:
                        level_sum += level
                        goals.remove(goal)
